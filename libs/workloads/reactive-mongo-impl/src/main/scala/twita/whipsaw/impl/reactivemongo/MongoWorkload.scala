...
object WorkloadDoc {
  implicit def fmt[SParams: OFormat, PParams: OFormat] =
    Json.format[WorkloadDoc[SParams, PParams]]
}

trait WorkloadDescriptor[Payload, SParams, PParams]
    extends ObjectDescriptor[EventId, Workload[Payload, SParams, PParams], WorkloadDoc[
      SParams,
      PParams
    ]] {
  ...
  implicit val dFmt: OFormat[WorkloadDoc[SParams, PParams]] =
    WorkloadDoc.fmt[SParams, PParams]
  ...
  override protected def cons
    : Either[Empty[WorkloadId], WorkloadDoc[SParams, PParams]] => Workload[
      Payload,
      SParams,
      PParams
    ] = { o =>
    new MongoWorkload[Payload, SParams, PParams](metadata, o)
  }
}

class MongoWorkload[Payload: OFormat, SParams: OFormat, PParams: OFormat](
  val metadata: Metadata[Payload, SParams, PParams],
  protected val underlying: Either[Empty[WorkloadId], WorkloadDoc[SParams,
                                                                  PParams]]
)(implicit executionContext: ExecutionContext,
  override val mongoContext: MongoContext with WorkloadContext)
    extends ReactiveMongoObject[EventId, Workload[Payload, SParams, PParams], WorkloadDoc[
      SParams,
      PParams
    ]]
  ...
  override def apply(
    ...
    case evt: ScheduleStatusUpdated =>
      update(SetOp(Json.toJsObject(evt)), evt, parent)
...
class MongoWorkloadFactory[Payload: OFormat, SParams: OFormat, PParams: OFormat](
  ...
  val mongoContext: MongoContext with WorkloadContext)
    extends ReactiveMongoDomainObjectGroup[EventId, Workload[
      Payload,
      SParams,
      PParams
    ], WorkloadDoc[SParams, PParams]]
    with WorkloadDescriptor[Payload, SParams, PParams]
  ...
  ): Future[List[Workload[Payload, SParams, PParams]]] = ???
