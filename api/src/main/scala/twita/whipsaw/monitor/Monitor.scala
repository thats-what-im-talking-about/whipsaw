...
object MonitorActor {
  def props[Attr](director: Director[Attr], out: ActorRef) =
    Props(new MonitorActor(director, out))
...
class MonitorActor[Attr](director: Director[Attr], out: ActorRef)
    extends Actor
    with ActorLogging {
  ...
  def activeState(data: Map[WorkloadId, WorkloadStatistics] = Map.empty,
    ...
    case MonitorActor.WorkloadReport(id, stats) if timer.isEmpty =>
      ...
      context.become(
        activeState(data + (id -> stats), setToReport + id, Some(newTimer))
      )
    ...
    case Some(workload: RegisteredWorkload[Attr]) =>
      context.actorOf(Props(new Probe(workload, director)))
